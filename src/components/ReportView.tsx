...
const ReportView: React.FC<ReportViewProps> = ({ historyEntries, categories, onClose }) => {
  ...
  useEffect(() => {
    try {
      ...
      currentMonthEntries.forEach(entry => {
        const day = new Date(entry.harvestDate).getDate() - 1;
  ...
  return (
    <div id="reportContainer" className="max-w-6xl mx-auto bg-white rounded-xl shadow-lg p-6 relative">
      <button onClick={onClose} className="absolute top-4 left-4 text-gray-500 hover:text-gray-700 flex items-center gap-1">
        <ArrowLeft className="h-5 w-5" /> <span>Back</span>
      </button>

      <div className="text-center mb-6 mt-4">
        <h2 className="text-2xl font-semibold text-gray-800">Brgy. Talipapa Farm and MRF Inventory Monthly Report</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-gray-50 rounded-lg p-4 shadow">
          ...
          <div className="h-64"><canvas ref={topItemsChartRef}></canvas></div>
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="bg-gray-50 rounded-lg p-4 shadow">
          ...
          <div className="h-64"><canvas ref={stockMovementChartRef}></canvas></div>
        </div>
      </div>

      <div className="flex gap-4">
        <button onClick={downloadReportAsPDF} className="flex-1 bg-teal-700 hover:bg-teal-800 text-white py-2 px-4 rounded-lg flex items-center justify-center gap-2">
          <Download className="h-5 w-5" /> Download as PDF
        </button>
        <button onClick={onClose} className="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-800 py-2 px-4 rounded-lg">
          Close
        </button>
      </div>
    </div>
